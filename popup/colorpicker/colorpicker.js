...
(function (window, document, undefined) {
  ...
  function $(el, attrs, children) {
    ...
    for (; i < len; i++) el.appendChild(children[i]);
    return el;
  }
  ...
  function slideListener(ctx, slideElement, pickerElement) {
    return function (evt) {
      ...
      ctx.callback &&
        ctx.callback(c.hex, { h: ctx.h - hueOffset, s: ctx.s, v: ctx.v }, { r: c.r, g: c.g, b: c.b }, undefined, mouse);
    };
  }
  ...
  function pickerListener(ctx, pickerElement) {
    return function (evt) {
      ...
      var c = hsv2rgb(ctx);
      ctx.callback &&
        ctx.callback(c.hex, { h: ctx.h - hueOffset, s: ctx.s, v: ctx.v }, { r: c.r, g: c.g, b: c.b }, mouse);
    };
  }
  ...
  function ColorPicker(slideElement, pickerElement, callback) {
    ...
    if (type == "SVG") {
      ...
      var blackAndWhiteGradients = [
        pickerClone.getElementsByTagName("linearGradient")[0],
        pickerClone.getElementsByTagName("linearGradient")[1],
      ];
